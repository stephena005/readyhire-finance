...
const getAuthError = (code, fallbackPrefix = 'Sign-in') => {
    return AUTH_ERROR_MESSAGES[code] || `${fallbackPrefix} failed (${code}). Check console for details.`;
};

export const AuthPage = () => {
    const { darkMode, refreshUser } = useApp();
    ...
    // All sign-in handlers let onAuthStateChanged (in AppContext) handle setting user state.
    // Do NOT call login() here — it causes a race condition with the auth listener.
    ...
    const handleGoogle = async () => {
        ...
        try {
            ...
            // onAuthStateChanged in AppContext will set user state and trigger navigation
        } catch (e) {
            setError(getAuthError(e.code, 'Google sign-in'));
        }
        setLoading(false);
    };
    ...
    const handleApple = async () => {
        ...
        try {
            ...
            setError(getAuthError(e.code, 'Apple sign-in'));
        }
        setLoading(false);
    };
    ...
    const handleEmail = async (e) => {
        ...
        try {
            if (mode === 'signup') {
                ...
                // updateProfile runs AFTER onAuthStateChanged fires, so the listener
                // in AppContext sees displayName as null. We call refreshUser() after
                // updateProfile to push the correct name into global state.
            ...
            setError(getAuthError(e.code, mode === 'signup' ? 'Sign-up' : 'Sign-in'));
        }
        setLoading(false);
    };
