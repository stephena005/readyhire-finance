...
import { FileText, Upload, ChevronLeft, Sparkles, Loader2 } from 'lucide-react';
...
export const CVOnboardingPage = ({ onComplete }) => {
    ...
    const handleFile = async (e) => {
        ...
        try {
            ...
            if (f.type === 'application/pdf') {
                ...
                if (clean.length < 100) {
                    setError('Could not read this PDF. It may be image-based. Please copy and paste your CV text instead.');
                    setCvFile(null);
                    setLoading(false);
                    return;
                }
    ...
    const parseCV = async () => {
        if (cvText.length < 100) return setError('More CV content needed (at least 100 characters).');
        setLoading(true);
        setError('');
        const r = await parseCVText(cvText);
        if (r?.error) {
            setError(r.error);
        } else if (r?.candidate_profile) {
            setParsedCV(r);
            setCvData(r);
            setStep(2);
        } else {
            setError('Could not parse CV. Please check the text and try again.');
        }
        setLoading(false);
    };
        ...
        <div className="max-w-2xl mx-auto">
            ...
            <Card hover={false} className="p-6 space-y-4">
                <div className={'border-2 border-dashed rounded-2xl p-8 text-center ' + (darkMode ? 'border-slate-600' : 'border-slate-300')}>
                    ...
                    <p className="text-xs opacity-50 mb-4">PDF or TXT files</p>
    ...
    // Step 2: Job description / role selection
    if (step === 2) return (
        <div className="max-w-2xl mx-auto">
            ...
            <Card hover={false} className="p-6 space-y-4">
                ...
                <button onClick={() => setStep(3)} disabled={jdMethod === 'paste' ? jdText.length < 50 : !selectedRole} className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold disabled:opacity-50">Continue →</button>
            </Card>
        </div>
    );
